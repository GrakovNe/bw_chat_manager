...
class DeleteService:
    ...
    async def delete(self, api: ChatApi, target: DeleteTarget, *, by: str) -> DeleteResult:
        try:
            ...
            return DeleteResult(ok=False, message=f"{self.settings.dup_delete_failed_reply} {exc}")

        logger.info(
            "repeat deleted: chat=%s message=%s by=%s", target.chat_id, target.message_id, by
        )
        return DeleteResult(ok=True, message=self.settings.dup_deleted_reply)
